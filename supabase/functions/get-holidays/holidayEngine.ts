...
import { applyObservedRule, ObservedRule } from './observedRules.ts';
...
/**
 * Derives movable Christian holidays based on Easter (Observed in SL)
 */
export function getChristianHolidays(
  year: number,
  policy: CountryObservedPolicy = SIERRA_LEONE_POLICY
): HolidayMetadata[] {
  const easter = calculateEaster(year);
  const baseHolidays = [
    { title: 'Good Friday', date: subDays(easter, 2) },
  ...
  baseHolidays.forEach((h) => {
    ...
    const observedResult = applyObservedRule(h.date, rule);

    if (observedResult.observed) {
      holidays.push({ title: h.title, date: h.date, isObserved: false });
      holidays.push({
...
export function getRuleBasedHolidays(
  ...
  const christian = getChristianHolidays(year, policy);
