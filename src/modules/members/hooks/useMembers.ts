...
export function useMembers(filters?: Partial<MemberFilters>): UseMembersResult {
  ...
  const fetchMembers = useCallback(async () => {
    try {
      ...
      let query = supabase.from('members').select('*'); // Fetch all fields
      ...
      // Resolve profile photo URLs for each member
      const mappedMembers: Member[] = await Promise.all(
        (data || []).map(async (row: any) => {
          const profilePhotoUrl = await resolveProfilePhotoUrl(row.profile_photo);
          return {
            id: row.id,
