...
export function useMemberFilters(initialBranchId?: string): UseMemberFiltersResult {
  ...
  // Sync branch filter when initialBranchId changes (e.g. from AdminContext)
  useEffect(() => {
    if (initialBranchId) {
  ...
  // Filter first-timers
  const getFilteredFirstTimers = useMemo(
  ...
  const calculateStats = useMemo(
    () =>
      (members: Member[], firstTimers: FirstTimer[]): MemberStats => {
