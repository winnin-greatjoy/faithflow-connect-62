                ...
                {mode === 'scan' ? (
                    <Card className="aspect-square lg:aspect-video rounded-[40px] border-none bg-neutral-900 shadow-2xl relative overflow-hidden flex flex-col items-center justify-center text-white ring-8 ring-primary/5">
                        ...
                        <AnimatePresence mode="wait">
                            {isScanning ? (
                                ...
                                >
                                    ...
                                    <p className="text-xs font-black uppercase tracking-widest opacity-60">Validating Token...</p>
                                </motion.div>
                            ) : (
                                <motion.div
                                    initial={{ opacity: 0 }}
                        ...
                        <div className="absolute bottom-6 left-6 flex items-center gap-2">
                            ...
                            <Badge className="bg-white/10 text-white/40 border-none text-[8px] font-black tracking-widest py-1">LOGS ENCRYPTED</Badge>
                        </div>
                        <Button variant="ghost" size="icon" className="absolute top-6 right-6 text-white/40 hover:text-white transition-all">
                    ...
                    <Card className="p-8 pb-4 rounded-[40px] border-none bg-white shadow-2xl flex flex-col h-[500px]">
                                        ...
                                        <Button
                                            ...
                                            onClick={async (e) => {
                                                ...
                                                setLastScanResult({
                                                    ...
                                                    message: result.offline ? "Offline check-in saved." : "Manual check-in confirmed.",
                                                    timestamp: new Date().toLocaleTimeString()
                                                });
                                                setTimeout(() => setLastScanResult(null), 3000);
                                            }}
                        ...
                        <div className="mt-4 pt-4 border-t border-primary/5">
                            <Button variant="ghost" className="w-full text-primary font-black text-[10px] uppercase tracking-widest h-10 rounded-xl">
                    ...
                    {lastScanResult && (
                        ...
                        >
                            <div className="h-20 w-20 rounded-[28px] bg-white/20 flex items-center justify-center shrink-0">
                                {lastScanResult.success ? <CheckCircle2 className="h-10 w-10 text-white" /> : <XCircle className="h-10 w-10 text-white" />}
                            </div>
                            <div>
                                <h3 className="text-2xl font-serif font-black leading-tight">{lastScanResult.name || "Access Denied"}</h3>
                                <p className="text-sm font-bold opacity-80 mt-1">{lastScanResult.message}</p>
                                <div className="mt-4 flex items-center gap-2">
                                    <Badge className="bg-white/20 border-none text-[8px] font-black tracking-widest py-1">TIME: {lastScanResult.timestamp}</Badge>
                                    {lastScanResult.success && <Badge className="bg-white/20 border-none text-[8px] font-black tracking-widest py-1">ZONE: MAIN PORTAL</Badge>}
                                </div>
                            </div>
                ...
                <Card className="p-8 rounded-[40px] border border-primary/5 bg-white shadow-2xl shadow-primary/5">
                    ...
                    <div className="mt-8 pt-6 border-t border-primary/5">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Sync Pipeline</span>
                            <span className={cn(
                                "text-[10px] font-black uppercase tracking-widest",
                                isSyncing ? "text-primary animate-pulse" : bufferSize > 0 ? "text-amber-500" : "text-emerald-500"
                            )}>
                                {isSyncing ? 'Synchronizing...' : bufferSize > 0 ? `${bufferSize} Pending` : 'Synchronized'}
                            </span>
                        </div>
