...
export function RolePermissionsDynamicForm({
  open,
  onClose,
  roleId,
  roleName,
  ...
  const [coverageType, setCoverageType] = useState<
    'global' | 'department' | 'ministry' | 'committee' | 'task'
  >('global');
  ...
  const ALL_ACTIONS: PermissionAction[] = useMemo(
  ...
  useEffect(() => {
    ...
    (async () => {
      const [mods, brs, deps, mins, cms, tks, feats] = await Promise.all([
        ...
        supabase
          .from('features')
          .select('id, module_id, slug, name, is_active')
          .eq('is_active', true),
      ]);
      if (!active) return;
      if (mods.error) {
        toast({
          ...
          description: mods.error.message,
          variant: 'destructive',
        });
        return;
      }
      ...
      if (deps.error) {
        toast({
          ...
          description: deps.error.message,
          variant: 'destructive',
        });
        return;
      }
      if (mins.error) {
        toast({
          ...
          description: mins.error.message,
          variant: 'destructive',
        });
        return;
      }
      ...
      if (feats.error) {
        toast({
          ...
          description: feats.error.message,
          variant: 'destructive',
        });
        return;
      }
      setModules((mods.data || []) as ModuleRow[]);
      setBranches((brs.data || []) as { id: string; name: string }[]);
      setDepartments((deps.data || []) as { id: string; name: string }[]);
      setMinistries((mins.data || []) as { id: string; name: string }[]);
      setCommittees((cms.data || []) as { id: string; name: string }[]);
      setTasks((tks.data || []) as { id: string; title: string }[]);
      const fmap: Record<string, FeatureRow[]> = {};
      const featsData = (feats.data || []) as FeatureRow[];
      featsData.forEach((f) => {
  ...
  useEffect(() => {
    ...
    if (coverageType === 'department' && !departmentId) {
      setSelections({});
      setFeatureSelections({});
      return;
    }
    if (coverageType === 'ministry' && !ministryId) {
      setSelections({});
      setFeatureSelections({});
      return;
    }
    if (coverageType === 'committee' && !committeeId) {
      setSelections({});
      setFeatureSelections({});
      return;
    }
    if (coverageType === 'task' && !taskId) {
      setSelections({});
      setFeatureSelections({});
      return;
    }
    ...
    (async () => {
      ...
      if (coverageType === 'department') {
        q = q
          .eq('department_id', departmentId)
          .is('ministry_id', null)
          .is('committee_id', null)
          .is('task_id', null);
      } else if (coverageType === 'ministry') {
        q = q
      ...
      (data || []).forEach((row: RolePermRow) => {
        const slug = row.module?.slug;
        if (!slug) return;
        const actionsArr = (row.actions || []) as (PermissionAction | string)[];
        const perms = actionsArr.map((a) => a as PermissionAction);
        if (row.feature_id) {
          nextFeat[row.feature_id] = new Set<PermissionAction>(perms);
        } else {
          next[slug] = new Set<PermissionAction>(perms);
        }
      });
      setSelections(next);
  ...
  const setAllForFeature = (featureId: string, value: boolean) => {
    setFeatureSelections((prev) => {
      ...
      next[featureId] = value
        ? new Set<PermissionAction>(ALL_ACTIONS)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let del = supabase
        ...
        .eq('scope_type', scopeType)
        .eq('coverage_type', coverageType);
      if (scopeType === 'branch') del = del.eq('branch_id', branchId);
      else del = del.is('branch_id', null);
      if (coverageType === 'department')
        del = del
          .eq('department_id', departmentId)
          .is('ministry_id', null)
      ...
      const bySlug: Record<string, ModuleRow> = Object.fromEntries(
        modules.map((m) => [m.slug as string, m])
      );
      const featuresById: Record<string, FeatureRow> = {};
      Object.values(featuresByModule).forEach((arr) =>
        arr.forEach((f) => {
          featuresById[f.id] = f;
        })
      );
      ...
      if (payload.length > 0) {
        const { error: insErr } = await supabase.from('role_permissions').insert(payload);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        ...
        <div className="space-y-5 overflow-y-auto flex-1 pr-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={scopeType}
                onValueChange={(v) => setScopeType(v as 'global' | 'branch')}
              >
            ...
            <div>
              ...
              <Select
                value={coverageType}
                onValueChange={(v) =>
                  setCoverageType(v as 'global' | 'department' | 'ministry' | 'committee' | 'task')
                }
              >
                <SelectTrigger className="w-full">
                          ...
                          <div key={f.id} className="border rounded-md p-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-2 text-xs">
                                <Checkbox
                                  checked={fall}
                                  onCheckedChange={(v) => setAllForFeature(f.id, Boolean(v))}
                                />
                                <span>All</span>
                                {fsome && !fall && (
                                  <span className="text-[10px] text-gray-500">Partial</span>
                                )}
                              </div>
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 mt-3">
        ...
        <div className="flex justify-end gap-2 pt-4 border-t bg-white shrink-0">
