...
export const MemberTrainingView = () => {
    ...
    const { data: trainingRecords, isLoading, refetch } = useQuery({
        ...
        `)
            ...
            let filtered = data as unknown as (MemberTraining & { member: { full_name: string, branch?: { name: string } } })[];
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            <TableRow key={record.id}>
                                ...
                                <TableCell className="capitalize">{record.training_type.replace(/_/g, " ")}</TableCell>
                                <TableCell>
                                    <Badge className={getStatusColor(record.status)}>
                                        {record.status.replace(/_/g, " ")}
                                    </Badge>
                                </TableCell>
                                <TableCell>{record.completed_at ? new Date(record.completed_at).toLocaleDateString() : "-"}</TableCell>
                                <TableCell>
                                    <Select
                                        value={record.status}
