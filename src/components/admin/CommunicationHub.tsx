  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
      ...
      <div>
        ...
        <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2">Send messages and stay connected with your congregation.</p>
      </div>
        ...
        <div className="w-full lg:w-2/3">
          <Card>
            <CardHeader className="p-3 sm:p-6 pb-0 sm:pb-0">
              ...
              <CardDescription className="text-xs sm:text-sm">Send messages to your congregation via email, SMS, or push notifications</CardDescription>
            </CardHeader>
            <CardContent className="p-3 sm:p-6 pt-3">
              <Tabs value={messageType} onValueChange={setMessageType} className="w-full">
                ...
                <TabsContent value="sms" className="space-y-3 sm:space-y-4 mt-4">
                  <div>
                    <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2">SMS Message (160 characters max)</label>
                    <Textarea 
                ...
                <TabsContent value="push" className="space-y-3 sm:space-y-4 mt-4">
                  <div>
                    <label className="block text-xs sm:text-sm font-medium mb-1 sm:mb-2">Notification Title</label>
                    <input 
                      type="text" 
                      className="w-full p-2 sm:p-2.5 text-sm border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" 
      ...
      <div className="w-full lg:w-1/3">
        <Card>
          <CardHeader className="p-3 sm:p-6 pb-0 sm:pb-0">
            ...
            <CardDescription className="text-xs sm:text-sm">Your recently sent messages</CardDescription>
          </CardHeader>
          <CardContent className="p-3 sm:p-6 pt-3">
              ...
              {recentMessages.map((message) => (
                <div key={message.id} className="border-l-2 border-blue-500 pl-3 py-2">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="font-medium text-sm sm:text-base line-clamp-1">{message.subject}</h3>
                  ...
                  <div className="mt-1.5">
                    <span className={`inline-flex items-center text-[10px] sm:text-xs px-2 py-0.5 sm:py-1 rounded-full ${
                      message.status === 'Delivered' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
                      {message.status}
                    </span>
                  </div>
