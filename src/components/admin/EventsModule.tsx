...
const getNextOccurrenceWithPattern = (ev: EventItem, fromDate = new Date()): string | null => {
  ...
  if (ev.frequency === 'Monthly') {
    ...
    let candidate = new Date(today.getFullYear(), today.getMonth(), dom === 'last' ? 1 : Number(dom));
    if (dom === 'last') {
    ...
    while (candidate < today && attempts < 120) {
      ...
      if (dom === 'last') candidate = new Date(candidate.getFullYear(), candidate.getMonth() + 1, 0);
      attempts++;
    }
    // ensure >= base
...
const generateOccurrencesWithinWithPattern = (ev: EventItem, days: number, fromDate = new Date()): string[] => {
  const out: string[] = [];
  ...
  if (ev.frequency === 'Weekly') {
    ...
    while (cur <= limitDate) {
      if (weekdays.includes(cur.getDay()) && cur >= parseDate(ev.date)) out.push(formatDateISO(cur));
      cur = addDays(cur, 1);
    }
    return out;
  }
  ...
  if (ev.frequency === 'Monthly') {
    ...
    while (cursor <= limitDate) {
      ...
      if (candidate >= start && candidate <= limitDate && candidate >= parseDate(ev.date)) out.push(formatDateISO(candidate));
...
const initialEvents: EventItem[] = [
  {
    ...
    attendeesList: [
      { id: 1, name: 'John Doe', memberLink: 'M-001', contact: '555-0101', role: 'Member', checkedIn: true, registeredAt: new Date().toISOString() },
      { id: 2, name: 'Jane Smith', memberLink: '', contact: '555-0102', role: 'Visitor', checkedIn: false, registeredAt: new Date().toISOString() },
    ],
  ...
  },
  {
    ...
    requiresRegistration: false,
    attendeesList: [
      { id: 10, name: 'Samuel K', memberLink: 'M-011', contact: '555-0201', role: 'Member', checkedIn: false, registeredAt: new Date().toISOString() },
    ],
...
export const EventsModule: React.FC = () => {
  ...
  // filters
  const filteredEvents = events.filter((e) => (tab === 'All' || e.type === tab) && (scopeFilter === 'All' || e.scope === scopeFilter));
  ...
  const openEdit = (ev: EventItem) => {
    ...
    if (ev.frequency === 'Weekly') {
      const sel: Record<number, boolean> = { 0: false, 1: false, 2: false, 3: false, 4: false, 5: false, 6: false };
      (pattern.weekdays ?? [parseDate(ev.date).getDay()]).forEach((wd) => { sel[wd] = true; });
  ...
  const saveCreate = () => {
    if (!form || !form.title || !form.date) { alert('Title and date required'); return; }
    // persist recurrencePattern from weekdaySelection if weekly
    const rec: RecurrencePattern = form.recurrencePattern ?? {};
    if (form.frequency === 'Weekly') {
      rec.weekdays = Object.entries(weekdaySelection).filter(([,v]) => v).map(([k]) => Number(k));
  ...
  const saveEdit = () => {
    ...
    setEvents((p) => p.map((ev) => (ev.id === form.id ? { ...(ev as EventItem), ...(form as EventItem), recurrencePattern: rec } : ev)));
    setDialog(null);
    setForm(null);
  };
  ...
  const registerAttendee = (eventId: number, a: Partial<Attendee>) => {
    if (!a.name || !a.name.trim()) { alert('Provide name'); return; }
    setEvents((prev) => prev.map((ev) => {
      if (ev.id !== eventId) return ev;
      const newAtt: Attendee = { id: Date.now(), name: String(a.name), memberLink: a.memberLink, contact: a.contact, role: a.role, checkedIn: false, registeredAt: new Date().toISOString() };
  ...
  const unregisterAttendee = (eventId: number, attendeeId: number) => {
    ...
    setEvents((prev) => prev.map((ev) => (ev.id !== eventId ? ev : { ...ev, attendeesList: (ev.attendeesList ?? []).filter((a) => a.id !== attendeeId) })));
  };
  const toggleCheckIn = (eventId: number, attendeeId: number) => {
    setEvents((prev) => prev.map((ev) => {
      if (ev.id !== eventId) return ev;
      return { ...ev, attendeesList: (ev.attendeesList ?? []).map((a) => (a.id === attendeeId ? { ...a, checkedIn: !a.checkedIn } : a)) };
    }));
  };

  // Export functions
  const exportAllRegistrationsCSV = () => {
    const rows: string[][] = [['Event ID', 'Event Title', 'Attendee ID', 'Name', 'MemberLink', 'Contact', 'Role', 'CheckedIn', 'RegisteredAt']];
    for (const ev of events) {
      for (const a of ev.attendeesList ?? []) {
        rows.push([String(ev.id), ev.title, String(a.id), a.name, a.memberLink ?? '', a.contact ?? '', a.role ?? '', String(!!a.checkedIn), a.registeredAt ?? '']);
      }
    }
  ...
  const exportEventRegistrationsCSV = (ev: EventItem) => {
    const rows: string[][] = [['Attendee ID', 'Name', 'MemberLink', 'Contact', 'Role', 'CheckedIn', 'RegisteredAt']];
    for (const a of ev.attendeesList ?? []) rows.push([String(a.id), a.name, a.memberLink ?? '', a.contact ?? '', a.role ?? '', String(!!a.checkedIn), a.registeredAt ?? '']);
  ...
  const recurrenceLabel = (ev: EventItem) => {
    ...
    if (ev.frequency === 'Weekly') {
      const days = (ev.recurrencePattern?.weekdays ?? [parseDate(ev.date).getDay()]).map((d) =>
    ...
    <div className="space-y-6 px-2 sm:px-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex gap-2">
          <Button size="sm" onClick={() => setDialog('calendar')}><Calendar className="mr-2" /> Calendar</Button>
          <Button size="sm" onClick={exportAllRegistrationsCSV}><FileText className="mr-2" />Export CSV</Button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={openCreate}><Plus className="mr-2" />Create Event</Button>
        </div>
      </div>
                ...
                <div className="flex justify-between items-start gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 text-xs text-gray-600 flex flex-wrap gap-3">
                      <div className="flex items-center"><Calendar className="mr-1 h-4 w-4" />{getNextOccurrenceWithPattern(ev) ?? '—'}</div>
                      <div className="flex items-center"><Clock className="mr-1 h-4 w-4" />{ev.time}</div>
                      <div className="flex items-center"><MapPin className="mr-1 h-4 w-4" />{ev.location}</div>
                      <div className="flex items-center"><Users className="mr-1 h-4 w-4" />{regs}/{ev.capacity ?? '—'}</div>
                      <Badge className="text-xs">{recurrenceLabel(ev)}</Badge>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-2">
                    <div className={`px-2 py-1 rounded text-xs ${getStatusColor(ev.status)}`}>{ev.status}</div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" onClick={() => openView(ev)}>View</Button>
                      <Button size="sm" variant="outline" onClick={() => openEdit(ev)}>Edit</Button>
                      <Button size="sm" variant="ghost" onClick={() => deleteEvent(ev.id)}>Delete</Button>
                    </div>
                    <div className="text-xs text-gray-500 mt-1">{checkRate}% checked in</div>
                    <div className="flex gap-1 mt-1">
                      <Button size="sm" variant="outline" onClick={() => exportEventRegistrationsCSV(ev)}><Download className="mr-1" />CSV</Button>
                      <Button size="sm" variant="outline" onClick={() => exportEventPDFHandler(ev)}><Download className="mr-1" />PDF</Button>
                    </div>
                    {upcoming.length > 0 && <div className="text-xs text-gray-500 mt-1">Next: {upcoming[0]}</div>}
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <Dialog open={dialog === 'calendar'} onOpenChange={() => setDialog(null)}>
        <DialogContent className="max-w-4xl">
          <DialogHeader><DialogTitle>Calendar</DialogTitle></DialogHeader>
          <div className="mt-3">
            {calendarLoaded && FullCalendar && dayGridPlugin ? (
              // @ts-ignore
              <FullCalendar
                ...
                eventClick={(arg: any) => {
                  ...
                  if (evId) {
                    ...
                    if (ev) { setDialog('view'); setActiveEvent(ev); }
                  }
                }}
              />
            ) : (
              <div className="h-80 flex items-center justify-center bg-gray-50 rounded text-gray-500">
                <div>
                  ...
                  <div className="text-xs">Install <code>@fullcalendar/react</code> and <code>@fullcalendar/daygrid</code> to enable calendar plotting.</div>
                </div>
              </div>
            )}
          </div>
      ...
      {/* Create/Edit dialog */}
      <Dialog open={dialog === 'create' || dialog === 'edit'} onOpenChange={() => { setDialog(null); setForm(null); }}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>{dialog === 'create' ? 'Create Event' : 'Edit Event'}</DialogTitle></DialogHeader>
          ...
          {form && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <Label>Type</Label>
                <Select value={form.type ?? 'General'} onValueChange={(v) => setForm({ ...form, type: v as EventType })}>
              ...
              <div>
                <Label>Scope</Label>
                <Select value={form.scope ?? 'Local'} onValueChange={(v) => setForm({ ...form, scope: v as Scope })}>
              ...
              <div>
                ...
                <Select value={form.frequency ?? 'One-time'} onValueChange={(v) => setForm({ ...form, frequency: v as Frequency })}>
              ...
              {form.frequency === 'Monthly' && (
                <div>
                  ...
                  <Input type="number" min={1} max={31} value={String(form.recurrencePattern?.dom ?? parseDate(form.date ?? nowIso).getDate())} onChange={(e) => setForm({ ...form, recurrencePattern: { ...(form.recurrencePattern ?? {}), dom: e.target.value === 'last' ? 'last' : Number(e.target.value) } })} />
                  <div className="text-xs text-gray-500 mt-1">Enter day number or type "last" in the field to use month last day.</div>
                </div>
              )}
              ...
              {form.frequency === 'Yearly' && (
                <>
                  <div>
                    ...
                    <Input type="number" min={1} max={12} value={String(form.recurrencePattern?.month ?? (parseDate(form.date ?? nowIso).getMonth() + 1))} onChange={(e) => setForm({ ...form, recurrencePattern: { ...(form.recurrencePattern ?? {}), month: Number(e.target.value) } })} />
                  </div>
                  <div>
                    <Label>Day</Label>
                    <Input type="number" min={1} max={31} value={String(form.recurrencePattern?.day ?? parseDate(form.date ?? nowIso).getDate())} onChange={(e) => setForm({ ...form, recurrencePattern: { ...(form.recurrencePattern ?? {}), day: Number(e.target.value) } })} />
                  </div>
                </>
              )}
              ...
              {form.type === 'Registration' && (
                <div>
                  ...
                  <Input type="number" value={String(form.capacity ?? '')} onChange={(e) => setForm({ ...form, capacity: Number(e.target.value) })} />
                </div>
              )}
              ...
              <div className="col-span-1 sm:col-span-2 flex justify-end gap-2 mt-3">
                <Button variant="outline" onClick={() => { setDialog(null); setForm(null); }}>Cancel</Button>
      ...
      {/* View dialog */}
      <Dialog open={dialog === 'view'} onOpenChange={() => { setDialog(null); setActiveEvent(null); }}>
          ...
          {activeEvent && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                <div><strong>Title:</strong> {activeEvent.title}</div>
                <div><strong>Scope:</strong> {activeEvent.scope}</div>
                <div><strong>Date:</strong> {activeEvent.date}</div>
                <div><strong>Frequency:</strong> {activeEvent.frequency} ({recurrenceLabel(activeEvent)})</div>
              ...
              <div>
                <div className="flex justify-between items-center">
                  <h4 className="font-medium">Attendees ({(activeEvent.attendeesList ?? []).length})</h4>
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => exportEventRegistrationsCSV(activeEvent)}>CSV</Button>
                    <Button size="sm" variant="outline" onClick={() => exportEventPDFHandler(activeEvent)}>PDF</Button>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {(activeEvent.attendeesList ?? []).length === 0 && (
                        <tr><td colSpan={5} className="p-2 text-xs text-gray-500">No attendees</td></tr>
                      )}
                        ...
                        <tr key={a.id} className="border-t">
                          ...
                          <td className="p-2 text-center">
                            <div className="flex items-center justify-center gap-2">
                              <Button size="sm" variant={a.checkedIn ? 'outline' : 'default'} onClick={() => toggleCheckIn(activeEvent.id, a.id)}>
                                {a.checkedIn ? 'Undo' : 'Check in'}
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => unregisterAttendee(activeEvent.id, a.id)}>Remove</Button>
                            </div>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="mt-3 border rounded p-3">
                  ...
                  <AttendeeRegisterForm onRegister={(payload) => registerAttendee(activeEvent.id, payload)} />
                </div>
              </div>
            </div>
          )}
...
const AttendeeRegisterForm: React.FC<{ onRegister: (a: Partial<Attendee>) => void }> = ({ onRegister }) => {
  const [payload, setPayload] = useState<Partial<Attendee>>({ name: '', memberLink: '', contact: '', role: 'Visitor' });
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      <div>
        ...
        <Input value={payload.memberLink ?? ''} onChange={(e) => setPayload({ ...payload, memberLink: e.target.value })} />
      </div>
      <div>
        <Label>Contact</Label>
        <Input value={payload.contact ?? ''} onChange={(e) => setPayload({ ...payload, contact: e.target.value })} />
      </div>
      <div>
        <Label>Role</Label>
        <Select value={payload.role ?? 'Visitor'} onValueChange={(v) => setPayload({ ...payload, role: v })}>
      ...
      <div className="col-span-1 sm:col-span-2 flex justify-end gap-2 mt-2">
        <Button variant="outline" onClick={() => setPayload({ name: '', memberLink: '', contact: '', role: 'Visitor' })}>Reset</Button>
        <Button onClick={() => { onRegister(payload); setPayload({ name: '', memberLink: '', contact: '', role: 'Visitor' }); }}>Register</Button>
      </div>
    </div>
  );
};
